...
output_file="/srv/rl2-lab/flash8/mbronars3/legibility/full_training_scan/unet_delta_default_sweep/unet_output_3.txt"
...
for ((i=0; i<num_samples; i++)); do
  ...
  python /srv/rl2-lab/flash8/mbronars3/ICRA/scripts/update_yaml_3.py $alpha $beta $gamma

  # Run your program with the current random parameter combination and capture the output
  python robomimicEval.py --config-dir /srv/rl2-lab/flash8/mbronars3/workspace/diffusion_policy_robo/diffusion_policy/config/final_sweep --config-name stack_lowdim_unet_3.yaml  hydra.run.dir='/srv/rl2-lab/flash8/mbronars3/legibility/runs/${now:%H.%M.%S}_${name}_${task_name}'
