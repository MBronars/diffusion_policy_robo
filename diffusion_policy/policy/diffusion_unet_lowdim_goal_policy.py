...
class DiffusionUnetLowdimPolicy(BaseLowdimPolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor], goal_list: List[torch.Tensor], guidance_weights: List[Float]) -> Dict[str, torch.Tensor]:
        ...
        guidance_weights: list of weights for each goal feature
        ...
        if self.obs_as_local_cond:
            ...
            obs_cond = nobs[:,:To].reshape(nobs.shape[0], -1)
            goal_cond = [ngoals[:, :To].reshape(ngoals.shape[0], -1) for ngoals in ngoals]
            global_cond = [torch.cat([obs_cond, goal], dim=-1) for goal in goal_cond]
            shape = (B, T, Da)
    ...
    def compute_loss(self, batch):
        ...
        if self.obs_as_local_cond:
            ...
            obs_cond = obs[:,:self.n_obs_steps,:].reshape(
                obs.shape[0], -1)
            goal_cond = goal[:,:self.n_obs_steps,:].reshape(
                goal.shape[0], -1)
